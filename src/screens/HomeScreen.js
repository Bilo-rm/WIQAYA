...
// تعريف ثوابت الألوان لتنسيق متسق
const COLORS = {
  primary: '#D1FF66',
  background: '#f3f4f6',
  white: '#FFFFFF',
...
const HomeScreen = () => {
  ...
  // متغيرات الحالة لإدارة حالة المكون
  ...
  // مولد بيانات عينة بناءً على نطاق الوقت
  ...
  // عامل التحجيم لمعدل ضربات القلب ليتناسب مع نفس مقياس ضغط الدم
  const heartRateScalingFactor = 2; // ضبط هذا العامل بناءً على نطاق بياناتك
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.graphContainer} {...panResponder.panHandlers}>
          <View style={styles.timeFilters}>
            {['أسبوع', 'شهر'].map((label, index) => (
              <TouchableOpacity
                key={index}
                style={[
                  styles.filterButton,
                  timeRange === ['weekly', 'monthly'][index] && styles.activeFilter,
                ]}
                onPress={() => setTimeRange(['weekly', 'monthly'][index])}
              >
          ...
          <LineChart
            data={{
              ...
              datasets: [
                ...
                {
                  data: generateChartData(timeRange).heartRate.map(hr => hr * heartRateScalingFactor), // تحجيم معدل ضربات القلب
            ...
            chartConfig={{
              ...
              yAxisLabel: '',
              yAxisSuffix: '',
              // تسميات المحور Y مخصصة
              yAxisInterval: 1, // اختياري، الافتراضي هو 1
            }}
            bezier
            style={styles.chart}
          />
        ...
        {/* حقل الإدخال للتفاعل مع الذكاء الاصطناعي */}
...
const styles = StyleSheet.create({
  ...
  // أنماط بطاقة تحليل الذكاء الاصطناعي
  aiCard: {
    backgroundColor: COLORS.transparent,
    padding: 16,
    borderRadius: 8,
